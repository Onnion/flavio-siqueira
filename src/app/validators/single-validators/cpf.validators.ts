...
export const validCpf = (cpf: string): boolean => {
  ...
  if (cpf) {
    ...
    result = length === 11 ? checkCpf(value) : false;
...
const checkCpf = (strCPF: string): boolean => {
  ...
  if (invalidsCpfs.includes(strCPF)) { return false; }
  ...
  if ((over === 10) || (over === 11))  { over = 0; }
  if (over !== parseInt(strCPF.substring(9, 10), 10) ) { return false; }
